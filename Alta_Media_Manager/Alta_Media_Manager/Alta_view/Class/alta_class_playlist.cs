...
namespace Alta_Media_Manager.Alta_view.Class
{
    public class alta_class_playlist
    {
        ...
        public List<alta_class_playlist_details> alta_details { get; set; }
        ...
        {
            this.alta_user = mysql_alta_helpper.getUserOfPlaylist(this.alta_id);
        }

        public void LoadDetails()
        {

            this.alta_details = new List<alta_class_playlist_details>();
            try
            {
                using (MySqlConnection conn = new MySqlConnection(CommonUtilities.config.getConnectionString()))
                {
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(insert_query, conn))
                    {
                        ...
                        if (Convert.ToInt32(tmp) > 0)
                        {
                            ...
                            {
                                this.alta_details = mysql_alta_helpper.getListPlayListDetails(reader, true);
                            }
                        }
                    }
                    conn.Close();
    ...
    public class alta_class_playlist_details
    {
        ...
        public DateTime alta_time_play { get { return _time_play; } set { _time_play = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, value.Hour, value.Minute, value.Second); } }
        private DateTime _timeend;
        public DateTime alta_time_end { get { return _timeend;} set { _timeend = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, value.Hour, value.Minute, value.Second); } }
